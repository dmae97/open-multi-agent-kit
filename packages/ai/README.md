...
```bash
npm install @earendil-works/omk-ai
```

TypeBox exports are re-exported from `@earendil-works/omk-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@earendil-works/omk-ai';
...
```typescript
import { Type, Tool, StringEnum } from '@earendil-works/omk-ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@earendil-works/omk-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@earendil-works/omk-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@earendil-works/omk-ai';
...
```typescript
import { getModel, stream } from '@earendil-works/omk-ai';
...
```typescript
import { getProviders, getModels, getModel } from '@earendil-works/omk-ai';
...
```typescript
import { Model, stream } from '@earendil-works/omk-ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@earendil-works/omk-ai';
...
```typescript
import { getModel, complete, Context } from '@earendil-works/omk-ai';
...
```typescript
import { Context, getModel, complete } from '@earendil-works/omk-ai';
...
- OAuth login flows are not supported in browser environments. Use the `@earendil-works/omk-ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from '@earendil-works/omk-ai';
...
```bash
npx @earendil-works/omk-ai login              # interactive provider selection
npx @earendil-works/omk-ai login anthropic    # login to specific provider
npx @earendil-works/omk-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@earendil-works/omk-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@earendil-works/omk-ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@earendil-works/omk-ai/oauth';
...
```typescript
import { getModel, complete } from '@earendil-works/omk-ai';
import { getOAuthApiKey } from '@earendil-works/omk-ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@earendil-works/omk-ai`
