...
import { Container, Text, truncateToWidth } from "@earendil-works/omk-tui";
 ...
 * {@link createLocalBashOperations}, every spawn is gated by
 * sandbox spawn builder: shell is denied when an OS sandbox backend is missing
 * under enforce mode, the environment is filtered, and available backends wrap
 * the spawned command. When omitted, local bash behavior is unchanged.
 */
...
export interface BashToolOptions {
	...
	/** Optional loadout access guard for scoped command execution. */
...
export function createBashToolDefinition(
	...
	return {
		...
		async execute(
			...
			assertLoadoutAccess(accessGuard, { operation: "execute", toolName: "bash", command: spawnContext.command });
			...
			// extension gate) is still hard-blocked. Only block-tier verdicts (rm -rf /,
			// mkfs, dd to a block device, fork bomb) stop here; confirm-tier policy is
			// owned by the command-safety gate.
