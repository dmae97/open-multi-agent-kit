...
export default function (omk: ExtensionAPI) {
	...
	omk.on("session_start", async (event, ctx) => {
		ctx.ui.setTitle(event.reason === "new" ? "OMK RPC Demo (new session)" : "OMK RPC Demo");
	...
	omk.registerCommand("rpc-editor", {
	...
	omk.registerCommand("rpc-prefill", {
