...
export default function (omk: ExtensionAPI) {
	omk.on("session_shutdown", async (_event, ctx) => {
		...
		const { stdout: status, code } = await omk.exec("git", ["status", "--porcelain"]);
		...
		const commitMessage = `[omk] ${firstLine.slice(0, 50)}${firstLine.length > 50 ? "..." : ""}`;

		// Stage and commit
		await omk.exec("git", ["add", "-A"]);
		const { code: commitCode } = await omk.exec("git", ["commit", "-m", commitMessage]);
