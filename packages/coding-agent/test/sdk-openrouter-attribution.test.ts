...
describe("createAgentSession provider attribution headers", () => {
	...
	beforeEach(() => {
		...
		originalTelemetryEnv = process.env.OMK_TELEMETRY;
		delete process.env.OMK_TELEMETRY;
	});
	...
	afterEach(() => {
		if (originalTelemetryEnv === undefined) {
			...
			process.env.OMK_TELEMETRY = originalTelemetryEnv;
		}
	...
	it("adds default attribution headers for OpenRouter models", async () => {
		...
		expect(headers?.["HTTP-Referer"]).toBe("https://github.com/dmae97/omk");
		expect(headers?.["X-OpenRouter-Title"]).toBe("omk");
	...
	it("adds default attribution headers for direct NVIDIA NIM endpoints", async () => {
		...
		expect(headers?.["X-BILLING-INVOKE-ORIGIN"]).toBe("OMK");
	});
	...
	it("adds OpenCode session headers", async () => {
		...
		expect(headers?.["x-opencode-client"]).toBe("omk");
	});
