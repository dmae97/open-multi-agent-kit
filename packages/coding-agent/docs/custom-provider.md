...
Extensions can register custom model providers via `omk.registerProvider()`. This enables:
...
export default function (omk: ExtensionAPI) {
  ...
  omk.registerProvider("my-provider", {
    name: "My Provider",
...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. omk waits for the factory before startup continues, so the provider is available during interactive startup and to `omk --list-models`.
...
Use `omk.unregisterProvider(name)` to remove a provider that was previously registered via `omk.registerProvider(name, ...)`:
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map omk levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
```typescript
omk.registerProvider("custom-api", {
  baseUrl: "https://api.example.com",
  apiKey: "$MY_API_KEY",
...
Credentials are persisted in `~/.omk/agent/auth.json`:
...
When a request exceeds the model's context window, omk can recover automatically by compacting the conversation and retrying. This recovery only kicks in if omk recognizes the failure as an overflow.
...
`message_end` runs before omk tracks the assistant message for auto-compaction, so the rewritten `errorMessage` is what omk checks. With this in place, omk will:
...
- Match a provider-specific pattern, not omk's generic overflow patterns. Rewriting rate-limit or throttling errors (`rate limit`, `too many requests`) would falsely trigger compaction instead of omk's normal retry-with-backoff path.
