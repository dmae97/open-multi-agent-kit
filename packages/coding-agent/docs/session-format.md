...
### Base Message Types (from omk-ai)
...
### Extended Message Types (from omk-coding-agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if omk-generated (legacy field name)
...
Session metadata (e.g., user-defined display name). Set via `/name`, `--name` / `-n`, or `omk.setSessionName()` in extensions.
