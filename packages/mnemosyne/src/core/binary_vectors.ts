...
export function getVecType(env: NodeJS.ProcessEnv = process.env): VecType {
	const value = (env.MNEMOSYNE_VEC_TYPE ?? "int8").trim().toLowerCase();
...
export class BinaryVectorStore {
	...
	static maximally_informative_binarization(embedding: readonly number[]): Uint8Array {
		return maximallyInformativeBinarization(embedding);
	}
	...
	static hamming_distance(binaryA: Uint8Array | ArrayBuffer, binaryB: Uint8Array | ArrayBuffer): number {
		return hammingDistance(binaryA, binaryB);
	}
	...
	static information_theoretic_score(distance: number, dim: number = EMBEDDING_DIM): number {
		return informationTheoreticScore(distance, dim);
	}
	...
	store_vector(memoryId: string, embedding: readonly number[]): void {
		this.storeVector(memoryId, embedding);
	}
	...
	search(queryEmbedding: readonly number[], topK = 10): BinaryVectorSearchResult[] {
		...
		const rows = this.conn
			.query(`SELECT memory_id, binary_vector, magnitude FROM ${this.tableName}`)
			.all() as VectorRow[];
		const results: BinaryVectorSearchResult[] = [];
		for (const row of rows) {
			const distance = hammingDistance(queryBinary, bytesFromBlob(row.binary_vector));
			results.push({
	...
	search_batch(queryEmbeddings: readonly (readonly number[])[], topK = 10): BinaryVectorSearchResult[][] {
		return this.searchBatch(queryEmbeddings, topK);
	}
	...
	delete_vector(memoryId: string): void {
...
export const maximally_informative_binarization = maximallyInformativeBinarization;
export const hamming_distance = hammingDistance;
export const information_theoretic_score = informationTheoreticScore;
export const cosine_similarity = cosineSimilarity;
export const quantize_int8 = quantizeInt8;
