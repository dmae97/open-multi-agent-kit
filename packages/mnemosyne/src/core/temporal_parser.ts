...
export function _resolve_relative_day(reference: Date, dayNameText: string, qualifier = "this"): Date {
...
export function parse_nl_date(text: string, reference?: QueryTime): ParsedNaturalDate | null {
	...
	if (m !== null) {
		...
		const d = _resolve_relative_day(ref, parsedDayName, qualifier);
	...
	if (m !== null) {
		const parsedDayName = m[2] as string;
		const d = _resolve_relative_day(ref, parsedDayName, "this");
...
export function extract_temporal(text: string, reference?: QueryTime): TemporalInfo {
	const result = parse_nl_date(text, reference);
	const tags: string[] = [];
...
export function extract_date_from_text(text: string, reference?: QueryTime): string | null {
	return extract_temporal(text, reference).event_date;
}

export const parseNlDate = parse_nl_date;
export const extractTemporal = extract_temporal;
export const extractDateFromText = extract_date_from_text;
