...
export class EpisodicGraph {
	...
	extract_gist(content: string, memory_id: string): Gist {
		return this.extractGist(content, memory_id);
	}
	...
	extract_facts(content: string, memory_id: string): Fact[] {
		return this.extractFacts(content, memory_id);
	}
	...
	store_gist(gist: Gist, memory_id: string): void {
		this.storeGist(gist, memory_id);
	}
	...
	get_gist(id: string): Gist | null {
		return this.getGist(id);
	}
	...
	store_fact(fact: Fact, memory_id: string, session_id = "default"): void {
		this.storeFact(fact, memory_id, session_id);
	}
	...
	get_edges(source: string | null = null): GraphEdge[] {
		return this.getEdges(source);
	}
	...
	find_related_memories(memory_id: string, depth = 2, edge_type = "", min_weight = 0): RelatedMemory[] {
		return this.findRelatedMemories(memory_id, depth, edge_type, min_weight);
	}
	...
	find_facts_by_subject(subject: string): Fact[] {
		return this.findFactsBySubject(subject);
	}
	...
	find_gists_by_participant(participant: string): Gist[] {
		return this.findGistsByParticipant(participant);
	}
	...
	score_memory_link(source_memory_id: string, target_memory_id: string): number {
		return this.scoreMemoryLink(source_memory_id, target_memory_id);
	}
	...
	ingest_memory(content: string, memory_id: string, options: IngestOptions = {}): IngestResult {
		return this.ingestMemory(content, memory_id, options);
	}
