...
export const DEFAULT_PLUGIN_DIR = join(homedir(), ".hermes", "mnemosyne", "plugins");
...
export class MnemosynePlugin {
	...
	constructor(config: PluginConfig = {}) {
		if (new.target === MnemosynePlugin) throw new TypeError("MnemosynePlugin is abstract");
		this.config = config;
		const ctor = this.constructor as typeof MnemosynePlugin;
		this.name =
...
export class LoggingPlugin extends MnemosynePlugin {
...
export class MetricsPlugin extends MnemosynePlugin {
...
export class FilterPlugin extends MnemosynePlugin {
...
export class CompressionPlugin extends MnemosynePlugin {
...
export class PluginManager {
	...
	private readonly instances = new Map<string, MnemosynePlugin>();
	...
	registerPlugin(name: string, pluginClass: PluginConstructor): void {
		if (typeof pluginClass !== "function" || !(pluginClass.prototype instanceof MnemosynePlugin)) {
			throw new TypeError("pluginClass must be a MnemosynePlugin subclass");
		}
