...
export function normalize_chat(text: string, options: { add_implicit_subjects?: boolean } = {}): string | null {
...
export function normalizeChat(text: string, options: { addImplicitSubjects?: boolean } = {}): string | null {
	return normalize_chat(text, { add_implicit_subjects: options.addImplicitSubjects });
}

export function normalize_batch(messages: string[]): (string | null)[] {
	return messages.map(message => normalize_chat(message));
}

export const normalizeBatch = normalize_batch;

export function extraction_rate(messages: string[]): ExtractionRate {
	const normalized = normalize_batch(messages);
	let survived = 0;
...
export const extractionRate = extraction_rate;
