...
import { type ConsolidatedFact, computeFactId, VeracityConsolidator } from "./veracity_consolidation";
...
export const polyphonic_recall_is_enabled = polyphonicRecallIsEnabled;
...
export class PolyphonicRecallEngine {
	...
	vectorVoice(queryEmbedding: readonly number[] | Float32Array | null): VoiceRecallResult[] {
		if (envDisabled("MNEMOSYNE_VOICE_VECTOR") || queryEmbedding === null) return [];
		...
		try {
			rows = this.db
				.query(`
					...
					WHERE em.superseded_by IS NULL AND (em.valid_until IS NULL OR em.valid_until > ?)
					LIMIT 50000
				`)
				.all(now, now) as EmbeddingRow[];
		} catch {
			return [];
		}
	...
	vector_voice(queryEmbedding: readonly number[] | Float32Array | null): VoiceRecallResult[] {
		return this.vectorVoice(queryEmbedding);
	}

	graphVoice(query: string): VoiceRecallResult[] {
		if (envDisabled("MNEMOSYNE_VOICE_GRAPH")) return [];
	...
	graph_voice(query: string): VoiceRecallResult[] {
		return this.graphVoice(query);
	}

	factVoice(query: string): VoiceRecallResult[] {
		if (envDisabled("MNEMOSYNE_VOICE_FACT")) return [];
		const results: VoiceRecallResult[] = [];
		for (const word of queryWords(query)) {
			const subject = word[0] === undefined ? word : word[0].toUpperCase() + word.slice(1);
			for (const fact of this.consolidator.get_consolidated_facts(subject, 0.5)) {
				results.push({
					memoryId: factMemoryId(fact),
	...
	fact_voice(query: string): VoiceRecallResult[] {
		return this.factVoice(query);
	}

	temporalVoice(query: string): VoiceRecallResult[] {
		if (envDisabled("MNEMOSYNE_VOICE_TEMPORAL") || !looksTemporal(query)) return [];
		...
		try {
			rows = this.db
				.query(`
					...
					FROM working_memory
					WHERE timestamp > ? AND superseded_by IS NULL AND (valid_until IS NULL OR valid_until > ?)
					ORDER BY timestamp DESC
					LIMIT 20
				`)
				.all(weekAgo, new Date().toISOString()) as TemporalRow[];
		} catch {
			return [];
		}
		const now = Date.now();
	...
	temporal_voice(query: string): VoiceRecallResult[] {
		return this.temporalVoice(query);
	}
	...
	combine_voices(...voiceResults: readonly VoiceRecallResult[][]): Map<string, PolyphonicResult> {
		return this.combineVoices(...voiceResults);
	}
	...
	diversity_rerank(results: ReadonlyMap<string, PolyphonicResult>, top_k: number): PolyphonicResult[] {
		return this.diversityRerank(results, top_k);
	}
	...
	estimate_similarity(a: PolyphonicResult, b: PolyphonicResult): number {
		return this.estimateSimilarity(a, b);
	}
	...
	assemble_context(results: readonly PolyphonicResult[], budget: number): PolyphonicResult[] {
		return this.assembleContext(results, budget);
	}
	...
	getStats(): Record<string, JsonValue> {
		...
		return {
			...
			consolidation_stats: this.consolidator.get_stats() as unknown as Record<string, JsonValue>,
		};
	}

	get_stats(): Record<string, JsonValue> {
	...
	private lookupMemory(memoryId: string): MemoryHydrationRow | null {
		const working = this.db
			.query(`
				...
				WHERE id = ?
			`)
			.get(memoryId) as MemoryHydrationRow | null;
...
function factMemoryId(fact: ConsolidatedFact): string {
	return fact.id ?? computeFactId(fact.subject, fact.predicate, fact.object);
}
...
export function getPolyphonicEngine(beam: BeamMemoryState): PolyphonicRecallEngine {
	...
	const engine = new PolyphonicRecallEngine({ db: beam.db, dbPath: beam.dbPath });
	beam.caches.polyphonicEngine = engine;
	return engine;
}

export const get_polyphonic_engine = getPolyphonicEngine;
...
export const polyphonic_recall = polyphonicRecall;
