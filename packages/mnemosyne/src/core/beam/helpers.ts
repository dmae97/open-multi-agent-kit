...
export function normalizeWeights(
	...
	let vw = Math.max(0, vecWeight ?? envNumber("MNEMOSYNE_VEC_WEIGHT", DEFAULT_WEIGHTS[0]));
	let fw = Math.max(0, ftsWeight ?? envNumber("MNEMOSYNE_FTS_WEIGHT", DEFAULT_WEIGHTS[1]));
	let iw = Math.max(0, importanceWeight ?? envNumber("MNEMOSYNE_IMPORTANCE_WEIGHT", DEFAULT_WEIGHTS[2]));
...
export function workingMemoryVecSearch(
	...
	if (queryEmbedding.length === 0) return [];
	try {
		const limit = process.env.MNEMOSYNE_BEAM_MODE ? 500_000 : 50_000;
		const rows = db
			.query(`
