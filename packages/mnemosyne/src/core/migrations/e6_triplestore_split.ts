...
export const has_pending_migration = hasPendingMigration;
...
function migrateRows(db: Database, rows: readonly TripleCandidateRow[]): number {
	...
	const insert = db.prepare(`
		INSERT INTO annotations (memory_id, kind, value, source, confidence, created_at)
		VALUES (?, ?, ?, ?, ?, ?)
	`);
	for (const row of rows) {
		insert.run(row.subject, row.predicate, row.object, row.source, row.confidence ?? 1.0, row.created_at);
	}
	return rows.length;
}
...
export function migrate(
	...
	try {
		...
		db.run("BEGIN");
		try {
			const written = migrateRows(db, classified.rows);
			db.run("COMMIT");
