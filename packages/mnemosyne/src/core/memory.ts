...
import {
	...
	type MnemosyneLlmRuntimeOptions,
...
function hasOwn(options: MnemosyneOptions, key: keyof MnemosyneOptions): boolean {
	return Object.hasOwn(options, key);
}

function resolveRuntimeOptions(options: MnemosyneOptions): ResolvedMnemosyneRuntimeOptions | undefined {
	...
	let llm: ResolvedMnemosyneRuntimeOptions["llm"];
...
let defaultInstance: Mnemosyne | null = null;
...
function resolveDbPath(options: MnemosyneOptions, bank: string): string | undefined {
...
function defaultFor(bank: string | null | undefined = null): Mnemosyne {
	...
	if (defaultInstance === null || defaultInstance.bank !== targetBank) {
		...
		defaultInstance = new Mnemosyne({ bank: targetBank });
	}
