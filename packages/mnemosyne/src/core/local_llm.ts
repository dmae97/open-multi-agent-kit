...
import {
	getMnemosyneRuntimeOptions,
	isPiAiModel,
	type MnemosyneLlmCompleteOptions,
	type MnemosyneLlmCompletion,
} from "./runtime_options";

const ENV_MODEL_REPO = process.env.MNEMOSYNE_LLM_REPO ?? "";
const ENV_MODEL_FILE = process.env.MNEMOSYNE_LLM_FILE ?? "";
...
function activeLlmOptions() {
	return getMnemosyneRuntimeOptions()?.llm;
}

function activeCustomCompletion(): MnemosyneLlmCompletion | undefined {
...
function llmEnabled(): boolean {
	...
	return envBool("MNEMOSYNE_LLM_ENABLED", true);
}
...
function llmMaxTokens(): number {
	...
	return envInt("MNEMOSYNE_LLM_MAX_TOKENS", 2048);
}

function llmContextTokens(): number {
	return envInt("MNEMOSYNE_LLM_N_CTX", 2048);
}
...
function hostLlmEnabled(): boolean {
	...
	return envBool("MNEMOSYNE_HOST_LLM_ENABLED", false);
}

function hostLlmContextTokens(): number {
	return envInt("MNEMOSYNE_HOST_LLM_N_CTX", 32000);
}
...
function llmBaseUrl(): string {
	...
	return stripTrailingSlash(env("MNEMOSYNE_LLM_BASE_URL"));
}
...
function llmModelName(): string {
	...
	return env("MNEMOSYNE_LLM_MODEL") || "local";
}
...
function llmApiKey(): string {
	...
	return env("MNEMOSYNE_LLM_API_KEY");
}

function sleepPrompt(): string {
	return env("MNEMOSYNE_SLEEP_PROMPT").trim();
}
...
function buildPrompt(memories: readonly string[], source = ""): string {
...
function buildHostPrompt(memories: readonly string[], source = ""): string {
	const custom = formatSleepPrompt(memories, source);
	if (custom !== null) {
		return custom;
	}
...
async function tryHostLlm(prompt: string, maxTokens: number, temperature: number): Promise<[boolean, string | null]> {
	...
	const raw = await callHostLlm(prompt, {
		...
		temperature,
		timeout: 15,
		provider: env("MNEMOSYNE_HOST_LLM_PROVIDER").trim() || null,
		model: env("MNEMOSYNE_HOST_LLM_MODEL").trim() || null,
	});
...
async function callRemoteLlm(prompt: string, temperature = 0.3): Promise<string | null> {
...
async function summarizeChunk(memories: readonly string[], source = ""): Promise<string | null> {
	...
	if (llmEnabled() && llmBaseUrl() !== "" && !envBool("MNEMOSYNE_FORCE_LOCAL", false)) {
...
export const _buildHostPrompt = buildHostPrompt;
export const _cleanOutput = cleanOutput;
export const _callRemoteLlm = callRemoteLlm;

export const llm_available = llmAvailable;
export const call_local_llm = callLocalLlm;
export const summarize_memories = summarizeMemories;
