...
export function isEnhancedRecallEnabled(env: Env = process.env): boolean {
	return env.MNEMOSYNE_ENHANCED_RECALL === "1";
}
...
export class QueryCache {
		...
		if (embedding !== undefined && embedding !== null && embedding.length !== 0) {
			...
			if (bestKey !== null) {
				...
				if (entry !== undefined) {
					...
					this.hits += 1;
					if (bestScore >= 0.88) this.tier2_hits += 1;
					else this.tier3_hits += 1;
	...
	stats(): QueryCacheStats {
		return {
			hits: this.hits,
			misses: this.misses,
			hit_rate: Math.round(this.hit_rate * 1000) / 1000,
			tier1_hits: this.tier1_hits,
			tier2_hits: this.tier2_hits,
			tier3_hits: this.tier3_hits,
			tier4_hits: this.tier4_hits,
...
export const query_cache_enabled = isQueryCacheEnabled;
