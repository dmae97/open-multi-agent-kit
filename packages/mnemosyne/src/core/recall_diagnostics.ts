...
export class RecallDiagnostics {
	...
	record_tier_hits(tier: RecallTier | string, hit_count: number): void {
		this.recordTierHits(tier, hit_count);
	}
	...
	record_fallback_used(options: { readonly wm?: boolean; readonly em?: boolean } = {}): void {
		this.recordFallbackUsed(options);
	}
	...
	record_call(options: { readonly truly_empty?: boolean; readonly trulyEmpty?: boolean } = {}): void {
		this.recordCall(options);
	}
	...
	fallback_rate(): { readonly wm: number; readonly em: number } {
...
export const get_recall_diagnostics = getRecallDiagnostics;

export function resetRecallDiagnostics(): void {
	getDiagnostics().reset();
}

export const reset_recall_diagnostics = resetRecallDiagnostics;
...
export const explain_recall_diagnostics = explainRecallDiagnostics;
