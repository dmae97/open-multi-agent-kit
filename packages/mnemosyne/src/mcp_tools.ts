...
export const TOOLS: readonly ToolDefinition[] = [
	{
		...
		description: "Store a durable memory in Mnemosyne.",
	...
	},
	{
		...
		description: "Return Mnemosyne memory statistics.",
		inputSchema: EMPTY_SCHEMA,
	},
	{
	...
	{ name: "mnemosyne_get", description: "Retrieve one memory by ID.", inputSchema: GET_SCHEMA },
	{
	...
	},
	{
		...
		description: "Export Mnemosyne memories to a JSON file.",
	...
	},
	{
		...
		description: "Import Mnemosyne memories from a JSON file.",
		inputSchema: IMPORT_SCHEMA,
	},
	{
		name: "mnemosyne_diagnose",
		description: "Run PII-safe diagnostics on the active Mnemosyne database.",
		inputSchema: EMPTY_SCHEMA,
	},
	{
...
function resolveBank(args: ToolArguments): string {
	return stringArg(args, "bank") || process.env.MNEMOSYNE_MCP_BANK || "default";
}

function bankDbPath(bank: string): string {
	if (bank === "default") return join(dataDir(), DEFAULT_DB_FILENAME);
	return join(dataDir(), "banks", bank, DEFAULT_DB_FILENAME);
}

function createBeam(args: ToolArguments, bank = resolveBank(args)): BeamMemory {
	const sessionId = process.env.MNEMOSYNE_SESSION_ID || `mcp_${bank}`;
	return new BeamMemory({
		sessionId,
		dbPath: bankDbPath(bank),
		authorId: optionalStringArg(args, "author_id") ?? process.env.MNEMOSYNE_AUTHOR_ID ?? null,
		authorType: optionalStringArg(args, "author_type") ?? process.env.MNEMOSYNE_AUTHOR_TYPE ?? null,
		channelId: optionalStringArg(args, "channel_id") ?? process.env.MNEMOSYNE_CHANNEL_ID ?? sessionId,
	});
}

function sharedBeam(): BeamMemory {
	const configured = process.env.MNEMOSYNE_SHARED_SURFACE_DB;
...
function handleGraphQuery(args: ToolArguments): ToolResult {
	...
	return withBeam(args, (_beam, bank) => ({
...
const TOOL_HANDLERS: Record<string, Handler> = {
	mnemosyne_remember: handleRemember,
	mnemosyne_recall: handleRecall,
	mnemosyne_shared_remember: handleSharedRemember,
	mnemosyne_shared_recall: handleSharedRecall,
	mnemosyne_shared_forget: handleSharedForget,
	mnemosyne_shared_stats: () => handleSharedStats(),
	mnemosyne_sleep: handleSleep,
	mnemosyne_stats: handleStats,
	mnemosyne_get_stats: handleStats,
	mnemosyne_invalidate: handleInvalidate,
	mnemosyne_validate: handleValidate,
	mnemosyne_get: handleGet,
	mnemosyne_triple_add: handleTripleAdd,
	mnemosyne_triple_query: handleTripleQuery,
	mnemosyne_scratchpad_write: handleScratchpadWrite,
	mnemosyne_scratchpad_read: handleScratchpadRead,
	mnemosyne_scratchpad_clear: handleScratchpadClear,
	mnemosyne_export: handleExport,
	mnemosyne_update: handleUpdate,
	mnemosyne_forget: handleForget,
	mnemosyne_import: handleImport,
	mnemosyne_diagnose: handleDiagnose,
	mnemosyne_graph_query: handleGraphQuery,
	mnemosyne_graph_link: handleGraphLink,
};
...
export function handle_tool_call(name: string, args: ToolArguments = {}): ToolResult {
...
export function get_tool_definitions(): readonly ToolDefinition[] {
