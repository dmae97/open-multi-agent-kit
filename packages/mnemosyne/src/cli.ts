...
function resolveDbPath(context?: CliContext): string {
	return context?.dbPath ?? (context?.dataDir ? join(context.dataDir, "mnemosyne.db") : configuredDbPath());
}
...
export const cmdExport: CommandHandler = (args, context) => {
	if (args.length === 0) usage("Usage: mnemosyne export <file.json>");
...
export const cmdImport: CommandHandler = (args, context) => {
	if (args.length === 0) usage("Usage: mnemosyne import <file.json>");
	...
	if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed))
		fail("Import file must contain a Mnemosyne export object", 1);
	return withMemory(context, memory => {
...
export const cmdRemember: CommandHandler = (args, context) => {
	if (args.length === 0) usage("Usage: mnemosyne store <content> [source] [importance]");
...
export const cmdRecall: CommandHandler = (args, context) => {
	if (args.length === 0) usage("Usage: mnemosyne recall <query> [top_k]");
	const query = args[0] ?? "";
...
export const cmdUpdate: CommandHandler = (args, context) => {
	if (args.length < 2) usage("Usage: mnemosyne update <memory_id> <new_content> [importance]");
...
export const cmdDelete: CommandHandler = (args, context) => {
	if (args.length === 0) usage("Usage: mnemosyne delete <memory_id>");
	const memoryId = args[0] ?? "";
	return withMemory(context, memory => {
...
export const cmdScratchpad: CommandHandler = (args, context) => {
	if (args.length === 0) usage("Usage: mnemosyne scratchpad <read|write|clear> [content]");
	const subcmd = args[0];
	return withMemory(context, memory => {
		if (subcmd === "read") {
		...
		if (subcmd === "write") {
			if (args.length < 2) usage("Usage: mnemosyne scratchpad write <content>");
...
export const cmdBank: CommandHandler = (args, context) => {
	if (args.length === 0) usage("Usage: mnemosyne bank <list|create|delete> [name]");
	...
	try {
		...
		if (subcmd === "create") {
			if (args.length < 2) fail("Usage: mnemosyne bank create <name>");
			const name = args[1] ?? "";
		...
		if (subcmd === "delete") {
			if (args.length < 2) fail("Usage: mnemosyne bank delete <name>");
			const name = args[1] ?? "";
...
export function printHelp(context?: CliContext): void {
	out(context, "Mnemosyne - Local AI Memory System\n");
	out(context, "Usage: mnemosyne <command> [args]\n");
...
export async function runCli(args: readonly string[] = Bun.argv.slice(2), context?: CliContext): Promise<number> {
	...
	if (!handler) {
		...
		err(context, "Run 'mnemosyne --help' for usage.");
		return 2;
	}
	try {
