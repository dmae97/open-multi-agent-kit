...
import { handleToolCall, TOOLS } from "../src/mcp_tools";
...
describe("provider all-tools parity", () => {
	...
	it("diagnose, validate, graph, and shared handlers return structured provider results", () => {
		...
		expect(handleToolCall("mnemosyne_graph_query", { seed_memory_id: memoryId, bank: "ops" }).error).toBe(
			"Episodic graph not available",
		);
		expect(
		...
		).toBe("Episodic graph not available");
