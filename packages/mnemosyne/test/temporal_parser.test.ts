...
describe("temporal parser", () => {
	...
	it("extracts ISO absolute dates", () => {
		const result = extract_temporal("Meeting was on 2026-05-15", REF);
	...
	it("rejects invalid ISO dates and falls through", () => {
		const result = extract_temporal("Bad leap day 2026-02-29", REF);
	...
	it("parses slash dates with Python's US/EU heuristic", () => {
		expect(extract_temporal("US date 05/20/2026", REF).event_date).toBe("2026-05-20");
		expect(extract_temporal("EU date 20/05/2026", REF).event_date).toBe("2026-05-20");
		expect(extract_temporal("Short year 5/20/26", REF).event_date).toBe("2026-05-20");
		expect(extract_temporal("Impossible 31/02/2026", REF).event_date).toBeNull();
	});

	it("parses named month dates using the reference year when omitted", () => {
		expect(extract_temporal("Shipped May 20, 2026", REF).event_date).toBe("2026-05-20");
		expect(extract_temporal("Shipped May 20th", REF).event_date).toBe("2026-05-20");
		expect(extract_temporal("Shipped Sep 7", REF).event_date).toBe("2026-09-07");
		expect(extract_temporal("Invalid Feb 30", REF).event_date).toBeNull();
	});

	it("extracts relative dates deterministically", () => {
		let result = extract_temporal("I had a meeting today", REF);
		expect(result.event_date).toBe("2026-05-20");
		expect(result.event_date_precision).toBe("day");
		expect(result.temporal_tags).toEqual(["2026-05-20", "wednesday"]);

		result = extract_temporal("I had a meeting yesterday", REF);
		expect(result.event_date).toBe("2026-05-19");
		expect(result.event_date_precision).toBe("day");
		expect(result.temporal_tags).toEqual(["2026-05-19", "tuesday", "yesterday"]);

		result = extract_temporal("I have a meeting tomorrow", REF);
	...
	it("extracts bare day references as this-most-recent day", () => {
		let result = extract_temporal("on Monday we discussed the API", REF);
		expect(result.event_date).toBe("2026-05-18");
		expect(result.temporal_tags).toEqual(["2026-05-18", "week-21-2026", "monday"]);

		result = extract_temporal("on Wednesday we discussed the API", REF);
		expect(result.event_date).toBe("2026-05-20");
		expect(result.temporal_tags).toEqual(["2026-05-20", "week-21-2026", "wednesday"]);
	});

	it("extracts week, month, and year references", () => {
		expect(extract_temporal("this week", REF)).toMatchObject({
			event_date: "2026-05-20",
			event_date_precision: "week",
			temporal_tags: ["week-21-2026", "this-week"],
		});
		expect(extract_temporal("last week", REF)).toMatchObject({
			event_date: "2026-05-13",
			event_date_precision: "week",
			temporal_tags: ["week-20-2026", "last-week"],
		});
		expect(extract_temporal("next week", REF)).toMatchObject({
			event_date: "2026-05-27",
			event_date_precision: "week",
			temporal_tags: ["week-22-2026", "next-week"],
		});
		expect(extract_temporal("last month", REF)).toMatchObject({
			event_date: "2026-04-01",
			event_date_precision: "month",
			temporal_tags: ["2026-04", "last-month"],
		});
		expect(extract_temporal("next month", REF)).toMatchObject({
			event_date: "2026-06-01",
			event_date_precision: "month",
			temporal_tags: ["2026-06", "next-month"],
		});
		expect(extract_temporal("last year", REF)).toMatchObject({
			event_date: "2025-01-01",
			event_date_precision: "year",
			temporal_tags: ["2025", "last-year"],
		});
		expect(extract_temporal("next year", REF)).toMatchObject({
	...
	it("handles month and year boundaries", () => {
		expect(extract_temporal("last month", new Date("2026-01-15T00:00:00Z")).event_date).toBe("2025-12-01");
		expect(extract_temporal("next month", new Date("2026-12-15T00:00:00Z")).event_date).toBe("2027-01-01");
	});

	it("extracts past intervals", () => {
		let result = extract_temporal("We deployed 2 days ago", REF);
		expect(result.event_date).toBe("2026-05-18");
		expect(result.event_date_precision).toBe("day");
		expect(result.temporal_tags).toEqual(["2026-05-18", "2-days-ago"]);

		result = extract_temporal("We deployed 3 hours ago", REF);
		expect(result.event_date).toBe("2026-05-20");
		expect(result.event_date_precision).toBe("day");
		expect(result.temporal_tags).toEqual(["2026-05-20", "3-hours-ago"]);

		result = extract_temporal("We deployed 2 weeks back", REF);
	...
	it("returns unknown when no temporal reference exists", () => {
		const result = extract_temporal("The database password is hunter2", REF);
		expect(result.event_date).toBeNull();
		expect(result.event_date_precision).toBe("unknown");
		expect(result.temporal_tags).toEqual([]);
	...
	it("uses the first date expression when multiple are present", () => {
		const result = extract_temporal("Deployed v2 on 2026-01-15 and v3 yesterday", REF);
		expect(result.event_date).toBe("2026-01-15");
		expect(result.primary_signal).toBe("2026-01-15");
	});

	it("extracts just the date string", () => {
		expect(extract_date_from_text("Deployed yesterday", REF)).toBe("2026-05-19");
		expect(extract_date_from_text("No date here", REF)).toBeNull();
	});

	it("treats date-only and timezone-less string references as UTC", () => {
		expect(extract_temporal("yesterday", "2026-05-20").event_date).toBe("2026-05-19");
		expect(extract_temporal("yesterday", "2026-05-20T02:00:00").event_date).toBe("2026-05-19");
		expect(extract_temporal("yesterday", "2026-05-20T02:00:00Z").event_date).toBe("2026-05-19");
	});
});
