...
import { PolyphonicRecallEngine, polyphonicRecall, polyphonicRecallIsEnabled } from "../src/core/polyphonic_recall";
...
function insertWorking(
	...
	beam.db.run(
		...
		[id, content, timestamp, beam.sessionId, importance, timestamp],
	);
}
function seedPolyphonicFixture(beam: BeamMemoryState): PolyphonicRecallEngine {
	...
	beam.db.run(
		`INSERT INTO consolidated_facts
			...
			VALUES ('m1', 'Alice', 'owns', 'durable launch checklist', 0.9, 2, ?, ?, '[]', 'likely_true')`,
		[new Date().toISOString(), new Date().toISOString()],
	);
	return engine;
}

const previousPolyphonic = process.env.MNEMOSYNE_POLYPHONIC_RECALL;

afterEach(() => {
	if (previousPolyphonic === undefined) delete process.env.MNEMOSYNE_POLYPHONIC_RECALL;
	else process.env.MNEMOSYNE_POLYPHONIC_RECALL = previousPolyphonic;
	delete process.env.MNEMOSYNE_VOICE_VECTOR;
	delete process.env.MNEMOSYNE_VOICE_GRAPH;
	delete process.env.MNEMOSYNE_VOICE_FACT;
	delete process.env.MNEMOSYNE_VOICE_TEMPORAL;
});

describe("PolyphonicRecallEngine", () => {
	it("reads the polyphonic recall gate per call", () => {
		delete process.env.MNEMOSYNE_POLYPHONIC_RECALL;
		expect(polyphonicRecallIsEnabled()).toBe(false);
		process.env.MNEMOSYNE_POLYPHONIC_RECALL = "0";
		expect(polyphonicRecallIsEnabled()).toBe(false);
		process.env.MNEMOSYNE_POLYPHONIC_RECALL = "1";
	...
	it("honors per-voice gates without producing fake-success results", () => {
		...
		try {
			const engine = seedPolyphonicFixture(beam);
			process.env.MNEMOSYNE_VOICE_VECTOR = "0";
			process.env.MNEMOSYNE_VOICE_GRAPH = "0";
			process.env.MNEMOSYNE_VOICE_TEMPORAL = "0";
			const results = engine.recall("Alice recent", [1, 0], 10);
