...
import { parse_query_time, temporal_boost } from "../src/core/beam/recall";
...
describe("temporal recall scoring", () => {
	it("computes temporal boost with decay, invalid timestamps, and future clamping", () => {
		...
		expect(temporal_boost("2026-04-28T12:00:00.000Z", queryTime, 24)).toBeGreaterThan(0.36);
		expect(temporal_boost("2026-04-28T12:00:00.000Z", queryTime, 24)).toBeLessThan(0.38);
		expect(temporal_boost("2026-04-26T12:00:00.000Z", queryTime, 24)).toBeLessThan(0.06);
		expect(temporal_boost("not-a-date", queryTime, 24)).toBe(0);
		expect(temporal_boost("2026-04-29T15:00:00.000Z", queryTime, 24)).toBe(1);
		expect(temporal_boost("2026-04-29T09:00:00+00:00", queryTime, 3)).toBeGreaterThan(0.36);
	});

	it("parses query time inputs and rejects invalid values", () => {
		expect(parse_query_time("2026-04-29").toISOString()).toBe("2026-04-29T00:00:00.000Z");
		expect(parse_query_time("2026-04-29T12:00:00").toISOString()).toBe("2026-04-29T12:00:00.000Z");
		expect(parse_query_time("2026-04-29T15:00:00+03:00").toISOString()).toBe("2026-04-29T12:00:00.000Z");
		expect(parse_query_time(new Date("2026-04-29T12:00:00.000Z")).toISOString()).toBe("2026-04-29T12:00:00.000Z");
		expect(() => parse_query_time("not-a-date")).toThrow();
		expect(() => parse_query_time(12345 as never)).toThrow();
	});
