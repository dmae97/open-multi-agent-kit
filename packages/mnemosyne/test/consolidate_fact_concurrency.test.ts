...
import { VeracityConsolidator } from "../src/core/veracity_consolidation";
import { closeQuietly } from "../src/db";

function withDb<T>(fn: (path: string, cons: VeracityConsolidator) => T): T {
	const dir = mkdtempSync(join(tmpdir(), "mnemosyne-veracity-concurrency-"));
...
describe("consolidate_fact SQLite serialization", () => {
	it("records repeated same-SPO observations as one row with compounded confidence", () => {
		withDb((_path, cons) => {
			const first = cons.consolidate_fact("Alice", "is", "developer", "stated", "src_a");
			const second = cons.consolidate_fact("Alice", "is", "developer", "stated", "src_b");
			const third = cons.consolidate_fact("Alice", "is", "developer", "stated", "src_c");
	...
	it("writes distinct SPOs through separate connections without dropping rows", () => {
		withDb((path, cons) => {
			...
			try {
				cons.consolidate_fact("Person0", "is", "engineer", "stated", "src_0");
				second.consolidate_fact("Person1", "is", "engineer", "stated", "src_1");
				second.consolidate_fact("Person2", "is", "engineer", "stated", "src_2");
				cons.consolidate_fact("Person3", "is", "engineer", "stated", "src_3");
	...
	it("participates in an outer transaction instead of starting a nested BEGIN", () => {
		withDb((_path, cons) => {
			...
			try {
				const fact = cons.consolidate_fact("Dan", "is", "designer", "stated", "src_x");
	...
	it("rolls back its own transaction when a mid-update error occurs", () => {
		withDb((_path, cons) => {
			...
			const original = cons.bayesian_update;
			cons.bayesian_update = () => {
				throw new Error("simulated mid-update failure");
			};
			try {
				expect(() => cons.consolidate_fact("Eve", "is", "scientist", "stated", "src_b")).toThrow("simulated");
			} finally {
				cons.bayesian_update = original;
			}
	...
	it("does not leak a fact insert when conflict recording fails", () => {
		withDb((_path, cons) => {
			...
			const original = cons._record_conflict;
			let calls = 0;
			cons._record_conflict = (...args: Parameters<VeracityConsolidator["_record_conflict"]>) => {
				calls += 1;
				if (calls === 2) throw new Error("simulated mid-loop failure");
				return original.apply(cons, args);
			};
			try {
				expect(() => cons.consolidate_fact("Kate", "is", "Z", "stated", "src_z")).toThrow("simulated");
			} finally {
				cons._record_conflict = original;
			}
