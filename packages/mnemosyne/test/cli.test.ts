...
function tempRoot(): string {
	return mkdtempSync(join(tmpdir(), "mnemosyne-ts-cli-"));
}
...
describe("CLI command handlers", () => {
	...
	it("stats prints working, episodic, triple, bank, and DB path counts", () => {
		const root = tempRoot();
		try {
			const dbPath = join(root, "mnemosyne.db");
	...
	it("reports usage, parse, operation, and unknown-command errors without tracebacks", async () => {
		...
		try {
			...
			expect(usageIo.stderr).toContain("Usage: mnemosyne store <content> [source] [importance]");
			...
			expect(unknownIo.stderr).toContain("Run 'mnemosyne --help' for usage.");
