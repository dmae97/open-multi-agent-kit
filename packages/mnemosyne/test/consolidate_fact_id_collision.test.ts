...
import { compute_fact_id, VeracityConsolidator } from "../src/core/veracity_consolidation";
import { closeQuietly } from "../src/db";

function withDb<T>(fn: (path: string, cons: VeracityConsolidator) => T): T {
	const dir = mkdtempSync(join(tmpdir(), "mnemosyne-veracity-"));
...
describe("compute_fact_id", () => {
	it("is deterministic and uses the stable SHA-256 framed format", () => {
		const id = compute_fact_id("Alice", "is", "developer");
		expect(compute_fact_id("Alice", "is", "developer")).toBe(id);
	...
	it("distinguishes long, separator-smuggled, and differently bucketed SPOs", () => {
		const ids = new Set([
			...
			compute_fact_id("a\x1f", "b", "c"),
			compute_fact_id("a", "\x1fb", "c"),
		]);
		expect(ids.size).toBe(6);
	});

	it("normalizes Unicode and rejects invalid components", () => {
		expect(compute_fact_id("café", "is", "open")).toBe(compute_fact_id("café", "is", "open"));
		expect(() => compute_fact_id("", "is", "developer")).toThrow("must be non-empty");
		expect(() => compute_fact_id("Alice", "", "developer")).toThrow("must be non-empty");
		expect(() => compute_fact_id("Alice", "is", "")).toThrow("must be non-empty");
		expect(() => compute_fact_id(null as unknown as string, "is", "developer")).toThrow("must be a str");
	});
});
...
describe("consolidate_fact id collision behavior", () => {
	it("stores hash ids and keeps distinct long-content facts", () => {
		withDb((_path, cons) => {
			...
			for (const row of rows) expect(row.id).toBe(compute_fact_id("EngineerLeadAlice", pred, row.object));
		});
	});
	...
	it("deduplicates by SPO so legacy ids are preserved while new rows use hashes", () => {
		withDb((_path, cons) => {
			...
			const result = cons.consolidate_fact("Eve", "is", "a lawyer", "stated", "mem_new");
			const rows = cons.conn
			...
			cons.consolidate_fact("Eve", "is", "a judge", "stated", "mem_other");
			const newRow = cons.conn
				.query("SELECT id FROM consolidated_facts WHERE subject = 'Eve' AND object = 'a judge'")
				.get() as { id: string };
			expect(newRow.id).toBe(compute_fact_id("Eve", "is", "a judge"));
		});
	});
	...
	it("resolves conflicts with compute_fact_id and rejects ambiguous winners", () => {
		withDb((_path, cons) => {
			...
			const conflict = cons.get_conflicts()[0];
			if (conflict === undefined) throw new Error("expected Grace conflict");

			cons.resolve_conflict(conflict.id, "cf_definitely_not_in_db_0000000000");
			expect(cons.get_conflicts()).toHaveLength(1);

			const winning = compute_fact_id("Grace", "is", "the CTO");
			cons.resolve_conflict(conflict.id, winning);
			expect(cons.get_conflicts()).toHaveLength(0);
