...
function withDb<T>(fn: (path: string, cons: VeracityConsolidator) => T): T {
	const dir = mkdtempSync(join(tmpdir(), "mnemosyne-veracity-siblings-"));
...
describe("VeracityConsolidator sibling write methods", () => {
	it("resolve_conflict has first-writer-wins semantics", () => {
		withDb((_path, cons) => {
			...
			cons.resolve_conflict(conflict.id, conflict.fact_a_id);
			cons.resolve_conflict(conflict.id, conflict.fact_b_id);
	...
	it("run_consolidation_pass resolves obvious high-confidence conflicts and nests safely", () => {
		withDb((_path, cons) => {
			for (let i = 0; i < 4; i += 1) cons.consolidate_fact("Eve", "is", "CEO", "stated", `src_high_${i}`);
	...
	it("get_consolidated_facts and stats exclude superseded facts", () => {
		withDb((_path, cons) => {
			cons.consolidate_fact("Nina", "owns", "service-a", "stated");
			cons.consolidate_fact("Nina", "owns", "service-b", "inferred");
			const conflict = cons.get_conflicts()[0];
			if (conflict === undefined) throw new Error("expected Nina conflict");
			cons.resolve_conflict(conflict.id, conflict.fact_a_id);

			const facts = cons.get_consolidated_facts("Nina", 0);
			expect(facts).toHaveLength(1);
			const fact = facts[0];
			if (fact === undefined) throw new Error("expected active Nina fact");
			expect(fact.id).toBe(conflict.fact_a_id);

			const stats = cons.get_stats();
