...
import { isEnhancedRecallEnabled, isQueryCacheEnabled, QueryCache } from "../src/core/query_cache";
...
describe("QueryCache", () => {
	...
	it("persists to sqlite when a db path is supplied", () => {
		const dir = mkdtempSync(join(tmpdir(), "mnemosyne-query-cache-"));
		try {
	...
	it("keeps enhanced recall and query cache disabled unless the Python env gate is set", () => {
		...
		expect(isQueryCacheEnabled(false, { MNEMOSYNE_ENHANCED_RECALL: "1" })).toBe(false);
