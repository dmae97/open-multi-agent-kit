...
import type { EpisodicGraph, RelatedMemory } from "../src/core/episodic_graph";

const previousProactive = process.env.MNEMOSYNE_PROACTIVE_LINKING;

afterEach(() => {
	if (previousProactive === undefined) delete process.env.MNEMOSYNE_PROACTIVE_LINKING;
	else process.env.MNEMOSYNE_PROACTIVE_LINKING = previousProactive;
});
...
describe("proactive memory linking", () => {
	...
	it("is disabled by default and can be toggled per remember call", () => {
		delete process.env.MNEMOSYNE_PROACTIVE_LINKING;
