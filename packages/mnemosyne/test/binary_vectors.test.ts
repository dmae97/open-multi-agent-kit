...
describe("binary vector helpers", () => {
	...
	it("normalizes MNEMOSYNE_VEC_TYPE with Python-compatible fallback", () => {
		expect(getVecType({ MNEMOSYNE_VEC_TYPE: "bit" })).toBe("bit");
		expect(getVecType({ MNEMOSYNE_VEC_TYPE: "int8" })).toBe("int8");
		expect(getVecType({ MNEMOSYNE_VEC_TYPE: "float32" })).toBe("float32");
		expect(getVecType({ MNEMOSYNE_VEC_TYPE: "bogus" })).toBe("float32");
