...
function tempDb(): string {
	const root = mkdtempSync(join(tmpdir(), "mnemosyne-ab-toggle-"));
	roots.push(root);
	return join(root, "mnemosyne.db");
}
...
describe("A/B polyphonic voice toggles", () => {
	it("treats falsy values as disabled with whitespace and case normalization", () => {
		...
		for (const value of falsyValues) {
			withEngine(engine => {
				process.env.MNEMOSYNE_VOICE_VECTOR = value;
				process.env.MNEMOSYNE_VOICE_GRAPH = value;
				process.env.MNEMOSYNE_VOICE_FACT = value;
				process.env.MNEMOSYNE_VOICE_TEMPORAL = value;
	...
	it("documents every in-scope polyphonic voice toggle in the source contract", () => {
		withEngine(engine => {
			...
			process.env.MNEMOSYNE_VOICE_FACT = "0";
