...
export function mergeDagNodeRouting(auto: DagNodeRouting, override: DagNodeRouting | undefined): DagNodeRouting {
  ...
  const skills = override.skills ? unique(override.skills) : auto.skills;
  const mcpServers = override.mcpServers ? unique(override.mcpServers) : auto.mcpServers;
  const tools = override.tools ? unique(override.tools) : auto.tools;
  const hooks = override.hooks ? unique(override.hooks) : auto.hooks;
  return {
    ...auto,
    ...override,
    ...
    assignedCapabilities: {
      ...
      tools,
      hooks,
      ...(auto.assignedCapabilities ?? {}),
      ...(override.assignedCapabilities ?? {}),
    },
...
export function dagNodeRoutingEnv(node: DagNode, dag?: import("./dag.js").Dag): Record<string, string> {
  ...
  return {
    ...
    OMK_NODE_PROVIDER: routing.provider ?? "auto",
    ...
    OMK_NODE_SKILLS: Array.from(skillHints).join(","),
    OMK_NODE_MCP_SERVERS: Array.from(mcpHints).join(","),
    OMK_NODE_TOOLS: Array.from(toolHints).join(","),
    OMK_NODE_HOOKS: Array.from(hookHints).join(","),
    OMK_PROVIDER_MODEL_TIER: routing.providerModelTier ?? "",
    OMK_PROVIDER_FALLBACK: routing.fallbackProvider ?? "",
