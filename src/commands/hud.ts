...
export function buildHudSidebar(
  ...
  if (state?.routeDecision) {
    lines.push(`  ${style.gray("route")} ${style.cream(truncateText(state.routeDecision.intent, 28))}`);
    lines.push(`  ${style.gray("mode")} ${style.cream(state.routeDecision.mode)}`);
    lines.push("");
  }

  const capabilityEntries = Object.entries(state?.capabilityAssignments ?? {});
  if (capabilityEntries.length > 0) {
    lines.push(`  ${style.pinkBold("CAPABILITY")}`);
    for (const [nodeId, assignment] of capabilityEntries.slice(0, 4)) {
      const prefix = truncateText(sanitizeForDisplay(nodeId), 12);
      if (assignment.skills?.length) lines.push(`  ${style.gray(prefix)} ${style.cream(truncateText(`skills:${assignment.skills.join(",")}`, 42))}`);
      if (assignment.hooks?.length) lines.push(`  ${style.gray(prefix)} ${style.cream(truncateText(`hooks:${assignment.hooks.join(",")}`, 42))}`);
      if (assignment.mcpServers?.length) lines.push(`  ${style.gray(prefix)} ${style.cream(truncateText(`mcp:${assignment.mcpServers.join(",")}`, 42))}`);
    }
    lines.push("");
  }
...
async function buildProviderUsagePanel(providerUsage?: ProviderUsageStats, fetchQuota = true): Promise<string> {
  ...
  return panel(planLines, gradient("Provider Usage"));
}
...
async function renderFullDashboard(options: HudRenderOptions): Promise<string> {
  ...
  mainPanels.push(await buildProviderUsagePanel(options.providerUsage, options.fetchQuota ?? true));
...
async function renderSectionDashboard(options: HudRenderOptions): Promise<string> {
  ...
  switch (options.section) {
    ...
    case "resources": {
      ...
      output.push("");
      output.push(await buildProviderUsagePanel(options.providerUsage, options.fetchQuota ?? true));
      break;
    }
  }
...
export async function hudCommand(options: HudCommandOptions = {}): Promise<void> {
  ...
  let cachedUsage: ProviderUsageStats | undefined;
  ...
  try {
    ...
    while (!stopped) {
      ...
      if (!cachedUsage || now - lastUsageRefreshMs >= usageRefreshMs) {
        cachedUsage = await getProviderUsage();
        lastUsageRefreshMs = now;
      }
      const frame = await renderHudDashboard({ ...options, providerUsage: cachedUsage, footerRefreshMs: refreshMs });
      if (frame !== lastFrame) {
        lastFrame = frame;
