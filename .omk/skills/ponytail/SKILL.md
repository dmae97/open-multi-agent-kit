...
metadata:
  ...
  vendored-source-path: "skills/ponytail/SKILL.md"
...
substance from `skills/ponytail/SKILL.md` at pinned commit
...
- **License:** MIT. Copyright (c) 2026 DietrichGebert. Full text: [`LICENSE`](LICENSE) in this
  directory (vendored copy) and the upstream `LICENSE` file at the pinned commit.
...
- **Invocation in OMK:** use `/skill:ponytail` or `!skill:ponytail` (shorthand `!ponytail` also
  works when unambiguous), per `packages/coding-agent/docs/skills.md`. The bare `/ponytail`
  slash form referenced above describes the upstream multi-host package's own command surface
  (Claude Code, Codex, OpenCode, etc.) and is not an OMK command.
