...
| Public proof bundles | `omk.proof-bundle.v1`, `npm run proof:check`, `npm run proof:index`, and five scoped RC hardening bundles exist as proof-gate scaffolding. | Expand toward replay/inspect, graph-audit, and evidence-block scenarios before broader stability claims; proof bundles must keep checksums, known limitations, and sanitized repo-relative artifacts. |
