...
| Public proof bundles | `omk.proof-bundle.v1`, `npm run proof:check`, `npm run proof:index`, and eight scoped RC hardening bundles now cover no-Kimi, provider/doctor, fallback, native safety, contract/version, evidence-block, replay/inspect, and graph-audit axes. | Expand toward a ten-bundle RC gallery with no-Kimi and fallback-routing scenario depth; proof bundles must keep checksums, known limitations, and sanitized repo-relative artifacts. |
